...
import { getFaqByIdApi, updateFaqApi } from "../../api/faqApi";
...
export default function EditFaq() {
  ...
  const fetchFaq = async () => {
    try {
      ...
      toast.error("Unable to load FAQ");
    }
  };
  ...
  const validate = () => {
    ...
    if (!formData.categoryId) err.categoryId = "FAQ Category is required";

    if (!formData.question.trim()) err.question = "Question is required";

    if (!formData.answer.trim()) err.answer = "Answer is required";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await updateFaqApi(id!, formData);
