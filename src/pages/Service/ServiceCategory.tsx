...
import {
  ...
  changeServiceCategoryStatusApi,
...
export default function ServiceCategory() {
  ...
  const canAdd = hasPermission("servicecategory", "add");
  const canEdit = hasPermission("servicecategory", "edit");
  const canDelete = hasPermission("servicecategory", "delete");
  ...
  const fetchCategories = async (pageNo = page, searchText = search) => {
    try {
      ...
      const response = await getServiceCategoriesApi(pageNo, 10, searchText);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isEdit) {
        response = await updateServiceCategoryApi(editId, formData);
      } else {
        response = await addServiceCategoryApi(formData);
      }
  ...
  const handleStatus = async (id: string) => {
    try {
      const response = await changeServiceCategoryStatusApi(id);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await deleteServiceCategoryApi(id);
  ...
  return (
    <>
      <PageBreadcrumb pageTitle="Service Category" />
