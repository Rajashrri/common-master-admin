...
export default function CoreTeamList() {
  ...
  const canAdd = hasPermission("team", "add");
  const canEdit = hasPermission("team", "edit");
  const canDelete = hasPermission("team", "delete");

  const fetchMembers = async (pageNo = page, searchText = search) => {
    try {
      setLoading(true);

      const response = await getCoreTeamApi(pageNo, 10, searchText);
  ...
  return (
    <>
      <PageBreadcrumb pageTitle="Team List" />

      <div className="space-y-6">
        <ComponentCard title="Team List">
