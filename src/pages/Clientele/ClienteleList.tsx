...
export default function ClienteleList() {
  ...
  const fetchClients = async (pageNo = page, searchText = search) => {
    try {
      setLoading(true);

      const response = await getClientApi(pageNo, 10, searchText);
  ...
  const canAdd = hasPermission("clientele", "add");
  const canEdit = hasPermission("clientele", "edit");
  const canDelete = hasPermission("clientele", "delete");
